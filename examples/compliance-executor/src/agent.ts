/**
 * Compliance Executor Agent
 *
 * A2A Agent with AWCP support for contract compliance audit and stamping.
 * Uses Synergy AI with injected compliance rules to enforce document
 * completeness before applying the protected official stamp.
 */
...
import { complianceAgentCard } from './agent-card.js';
import { ComplianceSynergyExecutor } from './synergy-executor.js';
...
function setupLogFile(logFile: string): void {
  ...
  console.log(`[ComplianceExecutor] Logging to ${logFile}`);
}
...
async function main() {
  ...
  const a2aExecutor = new ComplianceSynergyExecutor(synergyUrl, config.stampImagePath);
  ...
  app.get('/health', (_req, res) => {
    res.json({ status: 'ok', synergy: synergyUrl, stamp: config.stampImagePath });
  });
  ...
  app.listen(config.port, () => {
    console.log('');
    console.log('╔════════════════════════════════════════════════════════════╗');
    console.log('║         Compliance Executor Agent Started                  ║');
    console.log('╠════════════════════════════════════════════════════════════╣');
    ...
    console.log(`║  Stamp:       ${config.stampImagePath.slice(-43).padEnd(43)}║`);
    console.log('╚════════════════════════════════════════════════════════════╝');
    console.log('');
  });
}
