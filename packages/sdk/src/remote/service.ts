 ...
 * AWCP Remote Service
 *
 * Handles the AWCP delegation protocol on the Remote (Collaborator) side.
...
import { type AwcpRemoteConfig, type ResolvedAwcpRemoteConfig, resolveConfig } from './config.js';
 ...
 */
export interface AwcpRemoteServiceOptions {
  /** A2A agent executor */
 ...
 */
export class AwcpRemoteService {
  private executor: AgentExecutor;
  private config: ResolvedAwcpRemoteConfig;
  ...
  constructor(options: AwcpRemoteServiceOptions) {
    ...
    this.hostClient = new HostClient();
  }

  /**
   * Handle incoming AWCP message from Host
   */
  async handleMessage(
    message: AwcpMessage,
    hostUrl: string
    ...
    switch (message.type) {
      case 'INVITE':
        return this.handleInvite(message, hostUrl);
      case 'START':
        await this.handleStart(message, hostUrl);
        return null;
      case 'ERROR':
   ...
   */
  private async handleStart(start: StartMessage, hostUrl: string): Promise<void> {
    ...
    try {
      ...
      await this.hostClient.send(hostUrl, doneMessage);
      ...
      await this.hostClient.send(hostUrl, errorMessage).catch(console.error);
